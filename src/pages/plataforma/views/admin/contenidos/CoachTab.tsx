...
import { Plus, Search, Edit2, Trash2, Eye, EyeOff, Tag as TagIcon, LayoutGrid, List } from 'lucide-react';
import { getDB, CoachAdvice } from '../../../mock/data';
...
export const CoachTab: React.FC = () => {
  ...
  useEffect(() => {
    setAdvices(getDB().coachAdvices);
  }, []);

  const filtered = advices.filter(a => a.title.toLowerCase().includes(search.toLowerCase()) || a.category.toLowerCase().includes(search.toLowerCase()));

  const activeCount = advices.filter(a => a.active).length;
  const inactiveCount = advices.length - activeCount;
  const totalViews = advices.reduce((acc, a) => acc + a.views, 0);
  const topAdvice = [...advices].sort((a, b) => b.views - a.views)[0]?.title ?? 'Ninguno';
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '1.25rem' }}>
          {filtered.map(advice => (
            <div key={advice.id} className="card" style={{ padding: '1.25rem', display: 'flex', flexDirection: 'column' }}>
              ...
              <p style={{ fontSize: '0.85rem', color: 'var(--text-muted)', marginBottom: '1rem', flex: 1 }}>{advice.shortText}</p>
              ...
              <div style={{ borderTop: '1px solid var(--border-color)', paddingTop: '0.75rem', display: 'flex', justifyContent: 'flex-end', gap: '0.5rem' }}>
                <button style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-muted)' }} title="Cambiar estado">
                  {advice.active ? <Eye size={16} /> : <EyeOff size={16} />}
                </button>
                <button style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--primary-color)' }} title="Editar">
                  <Edit2 size={16} />
                </button>
                <button style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--red-color)' }} title="Eliminar">
                  <Trash2 size={16} />
                </button>
              </div>
            </div>
            ...
            <tbody>
              {filtered.map(advice => (
                <tr key={advice.id} style={{ borderBottom: '1px solid var(--border-color)', fontSize: '0.9rem' }}>
                  ...
                  <td style={{ padding: '1rem', color: 'var(--text-muted)' }}>{advice.views}</td>
                  ...
                  <td style={{ padding: '1rem', textAlign: 'right', display: 'flex', justifyContent: 'flex-end', gap: '0.75rem' }}>
                    ...
                    <button style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--primary-color)' }}><Edit2 size={16} /></button>
                    <button style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--red-color)' }}><Trash2 size={16} /></button>
                  </td>
                </tr>
              ))}
            </tbody>
